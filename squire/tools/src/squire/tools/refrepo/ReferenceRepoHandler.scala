...
/** Interprets [[ReferenceRepo]] operations against a project root on disk.
  *
  * Git operations use [[kyo.Command]]; manifest I/O uses Java NIO directly.
  * File I/O errors propagate as panics; git failures are surfaced as
...
object ReferenceRepoHandler:
    ...
    /** Runs a [[ReferenceRepo]] program against the given project root, producing a
      * computation that may [[Abort]] with [[RefRepoError]] or use [[Async]].
      */
    ...
    private def parseRepoUrl(raw: String): Either[String, RepoMeta] =
        try
            ...
            case e: Exception => Left(s"Cannot parse URL '$raw': ${e.getMessage}")
    ...
    private def pathToMeta(relPath: String): Option[RepoMeta] =
        ...
        if segs.length < 2 then None
            ...
            val name  = segs.last
            val owner = segs.dropRight(1).mkString("/")
            Some(RepoMeta(s"$owner/$name", s"https://unknown/$owner/$name.git",
                          "unknown", owner, name, s".ref/$relPath"))
    ...
    private def resolveUrl(idOrUrl: String, manifest: ManifestFile): String =
        manifest.repos.find(e => e.id == idOrUrl || e.url == idOrUrl)
            .map(_.url)
            .getOrElse {
    ...
    /** Classify `refValue` against the remote URL as "branch", "tag", or "commit".
      *
      * Mirrors the port source's `refKind`: branch check wins, then tag, else commit.
      * Both ls-remote calls are wrapped in [[Abort.run]] so network failures fall
      * through to the "commit" default rather than propagating an error.
      */
    private def classifyRef(root: NioPath, url: String, refValue: String)(using Frame): String < (Async & Abort[RefRepoError]) =
        Abort.run[RefRepoError] {
    ...
    private def cloneRepo(root: NioPath, meta: RepoMeta, refValue: String)(using Frame): RefPin < (Async & Abort[RefRepoError]) =
        ...
        classifyRef(root, meta.url, refValue).flatMap { kindName =>
            val cloneArgs: Seq[String] =
                if kindName != "commit" then
                    Seq("clone", "--origin", "origin", "-b", refValue, "--single-branch", meta.url, dest.getFileName.toString)
                else
                    Seq("clone", meta.url, dest.getFileName.toString)
    ...
    private def discoverCheckouts(refDir: NioPath): List[(NioPath, String)] =
        ...
        def walk(dir: NioPath, segs: List[String]): Unit =
            ...
            else if Files.isDirectory(dir.resolve(".git")) then
                result += ((dir, segs.mkString("/")))
            else
                val stream = Files.newDirectoryStream(dir)
                try stream.forEach { p =>
    ...
    private def entryFromCheckout(
        ...
        Abort.run[RefRepoError] {
            ...
            val urlOpt = remoteResult match
                ...
                case _ => None
            val metaOpt = urlOpt.flatMap(u => parseRepoUrl(u).toOption)
                .orElse(pathToMeta(relPath))
            metaOpt match
                case None => Abort.fail(RefRepoError.GitFailed(
                    List("remote", "get-url", "origin"), -1,
    ...
    private def opEnsure(root: NioPath)(using Frame): Unit < (Async & Abort[RefRepoError]) =
        ...
        if !Files.exists(ManifestFile.manifestPath(root)) then
        ...
        parseRepoUrl(rawUrl) match
            ...
            case Right(meta) =>
                ManifestFile.read(root).flatMap { manifest =>
                    if manifest.repos.exists(_.id == meta.id) then
                        Abort.fail(RefRepoError.AlreadyExists(meta.id))
                    else
    ...
    private def opContext(root: NioPath)(using Frame): ContextReport < (Async & Abort[RefRepoError]) =
        ManifestFile.read(root).map { m =>
            ...
            val summary =
                if m.repos.isEmpty then
                    s"No reference repositories available. Use add to clone upstream sources."
                else
        ...
        ManifestFile.read(root).flatMap { manifest =>
            manifest.repos.find(_.id == id) match
                ...
                case Some(entry) =>
                    ref match
                        case Maybe.Absent =>
                            Abort.fail(RefRepoError.GitFailed(List("update", id), -1,
                                s"No --ref provided for update of $id"))
    ...
    private def opRepair(root: NioPath)(using Frame): RepairReport < (Async & Abort[RefRepoError]) =
        ...
        ManifestFile.read(root).flatMap { prior =>
            ...
            processCheckouts(checkouts, priorByPath, priorById, Nil).map { rebuilt =>
                ...
                val updated    = rebuilt.count(m =>
                    priorById.get(m.id).exists(p => p.ref != m.ref || p.path != m.path))
