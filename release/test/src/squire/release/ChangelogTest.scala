...
class ChangelogTest extends Test[Any]:
    ...
    "Changelog" - {
        ...
        "promote moves Unreleased and prepends fresh Unreleased; idempotent" in
            Abort.run[String](Changelog.parse(sample)).map {
                case Result.Success(cl) =>
                    val moved = cl.copy(sections =
                        cl.sections.map(s =>
                            if s.version == "Unreleased" then
                                s.copy(buckets =
                                    s.buckets.map(b => if b.name == "Added" then b.copy(entries = Chunk("New.")) else b)
                                )
                            else s
                        )
                    )
        ...
        "readiness counts Unreleased entries" in
            Abort.run[String](Changelog.parse(sample)).map {
                case Result.Success(cl) =>
                    val withEntries = cl.copy(sections =
                        cl.sections.map(s =>
                            if s.version == "Unreleased" then
                                s.copy(buckets =
                                    s.buckets
                                        .map(b => if b.name == "Added" then b.copy(entries = Chunk("A", "B")) else b)
                                )
                            else s
                        )
                    )
        ...
        "readiness fails when Unreleased is missing a canonical bucket" in
            Abort.run[String](Changelog.parse(sample)).map {
                case Result.Success(cl) =>
                    val dropFixed = cl.copy(sections =
                        cl.sections.map(s =>
